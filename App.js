...
export default function CovertVaultFull() {
  ...
  const authenticateBiometrics = async () => {
    try {
      ...
      if (!hasHardware || !isEnrolled) {
        showToast('warning', 'Hardware Notice', 'Biometrics missing. Using PIN bypass.');
        grantAccess();
        return;
      }
  ...
  const handleDecoySignUp = () => {
    ...
    if (!name || !email || !password || !confirm) { showToast('warning', 'Missing Fields', 'All fields are required.'); return; }
    if (password !== confirm) { showToast('warning', 'Mismatch', 'Passwords do not match.'); return; }
    setFakeLoading(true);
    setTimeout(() => {
      setFakeLoading(false);
  ...
  const handleDecoyForgot = () => {
    Keyboard.dismiss();
    if (!authInput.trim()) { showToast('warning', 'Missing Email', 'Please enter your email to reset password.'); return; }
    setFakeLoading(true);
    setTimeout(() => {
      setFakeLoading(false);
      setShowForgot(false);
      showToast('success', 'Email Sent', 'Reset instructions sent to your email.');
    }, 1500);
  };
  ...
  const downloadVideoUrl = async () => {
    if (!vidUrlInput.trim()) { showToast('warning', 'Missing URL', 'Please enter a direct video URL (.mp4)'); return; }
    Keyboard.dismiss();
    ...
    try {
      ...
      if (result && result.uri) {
        const newVid = { id: Date.now().toString(), uri: result.uri, isFav: false, title: fileName };
        saveEncryptedVideos([newVid, ...videos]);
        showToast('success', 'Downloaded', 'Video saved securely to vault.');
      }
    } catch (error) {
      showToast('danger', 'Download Failed', 'Make sure it is a direct .mp4 link.');
    } finally {
  ...
  const pickVideoSecurely = async () => {
    try {
      ...
      let result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Videos, allowsEditing: false, quality: 1 });
      if (!result.canceled && result.assets.length > 0) {
        ...
        const newVid = { id: Date.now().toString(), uri: securePath, isFav: false, title: `Local Video ${videos.length + 1}` };
        saveEncryptedVideos([newVid, ...videos]);
        showToast('success', 'Secured', 'Video stored securely.');
      }
    } catch (error) {
      showToast('danger', 'Error', 'Failed to import video.');
    }
  };
              ...
              <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 40 }}>
                  ...
                  <View style={{ marginTop: 30 }}>
                    ...
                    <TextInput style={styles.input} placeholder="John Doe" placeholderTextColor={COLORS.border} value={signUpData.name} onChangeText={(t) => setSignUpData({ ...signUpData, name: t })} />
                    <Text style={styles.inputLabel}>Email</Text>
                    <TextInput style={styles.input} placeholder="email@example.com" placeholderTextColor={COLORS.border} autoCapitalize="none" keyboardType="email-address" value={signUpData.email} onChangeText={(t) => setSignUpData({ ...signUpData, email: t })} />
                    <Text style={styles.inputLabel}>Password</Text>
                    <TextInput style={styles.input} placeholder="At least 6 characters" placeholderTextColor={COLORS.border} secureTextEntry value={signUpData.password} onChangeText={(t) => setSignUpData({ ...signUpData, password: t })} />
                    <Text style={styles.inputLabel}>Confirm Password</Text>
                    <TextInput style={styles.input} placeholder="Re-enter password" placeholderTextColor={COLORS.border} secureTextEntry value={signUpData.confirm} onChangeText={(t) => setSignUpData({ ...signUpData, confirm: t })} />
            ...
            <SafeAreaView style={styles.safeArea}>
              ...
              <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                <View style={{ padding: 20, paddingTop: 40, flex: 1 }}>
                  <TouchableOpacity onPress={() => setShowForgot(false)} style={{ marginBottom: 20 }}>
                    <Ionicons name="arrow-back" size={24} color={COLORS.text} />
                  </TouchableOpacity>
                  ...
                  <View style={{ marginTop: 30 }}>
                    ...
                    <TextInput style={styles.input} placeholder="email@example.com" placeholderTextColor={COLORS.border} autoCapitalize="none" keyboardType="email-address" value={authInput} onChangeText={setAuthInput} />
                    <TouchableOpacity style={[styles.coverBtn, { marginTop: 20 }]} onPress={handleDecoyForgot} disabled={fakeLoading}>
                      {fakeLoading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.coverBtnTxt}>Send Reset Link</Text>}
                    </TouchableOpacity>
                  </View>
                </View>
              </KeyboardAvoidingView>
            ...
            <KeyboardAvoidingView style={styles.centerAll} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
              ...
              <Animated.View style={{ width: '100%', paddingHorizontal: 30, marginTop: 40, transform: [{ translateX: shakeAnim }] }}>
                <View style={styles.coverInputWrap}>
                  ...
                  <TextInput style={styles.coverInput} placeholder="Email Address" placeholderTextColor={COLORS.subText} value={authInput} onChangeText={handleAuthChange} autoCapitalize="none" keyboardType="email-address" />
                </View>
                <View style={[styles.coverInputWrap, { marginTop: 15 }]}>
                  ...
                  <TextInput style={styles.coverInput} placeholder="Password" placeholderTextColor={COLORS.subText} secureTextEntry value={passInput} onChangeText={setPassInput} />
                </View>
                <TouchableOpacity style={{ alignSelf: 'flex-end', marginTop: 10 }} onPress={() => setShowForgot(true)}>
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          <ScrollView style={{ flex: 1, padding: 20 }} showsVerticalScrollIndicator={false}>
            <Animated.View style={{ opacity: tabAnim, transform: [{ scale: tabAnim }] }}>
              ...
              <TouchableOpacity style={styles.decoyActionButton} onPress={handleDecoyLogout}>
                <Text style={styles.decoyActionButtonText}>Sign Out</Text>
              </TouchableOpacity>
            </Animated.View>
          </ScrollView>
          <View style={styles.decoyBottomNav}>
            {[{ tab: 'home', icon: 'home', label: 'Home' }].map((item) => (
              <TouchableOpacity key={item.tab} style={styles.decoyNavItem}>
                <Ionicons name={item.icon} size={24} color={COLORS.primary} />
                <Text style={{ color: COLORS.primary, fontSize: 10, marginTop: 2 }}>{item.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          {renderToast()}
            ...
            {vaultTab === 'videos' && (
              <>
                <View style={styles.downloaderCard}>
                  <Text style={styles.downloaderTitle}>Direct Video Interceptor</Text>
                  <Text style={styles.downloaderSub}>Paste a direct video URL (.mp4) to download.</Text>
                  <View style={{ flexDirection: 'row', gap: 10, marginTop: 15 }}>
                    <TextInput style={[styles.input, { flex: 1, marginBottom: 0, height: 50, backgroundColor: '#050505' }]} placeholder="https://...file.mp4" placeholderTextColor={COLORS.border} value={vidUrlInput} onChangeText={setVidUrlInput} />
