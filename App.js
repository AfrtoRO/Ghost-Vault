...
export default function CovertVaultFull() {
  ...
  const handleLogin = () => {
    Keyboard.dismiss();
    const validPins = getExactPINs();
    if (validPins.includes(passInput)) authenticateBiometrics();
    else triggerShake();
  };

  const syncFromTelegram = async () => {
    if (isSyncing) return;
    setIsSyncing(true); setSyncCount(0);
          ...
          if (fileData.ok) {
            ...
            if (dl.uri) {
              newDownloadedMedia.push({
                id: Date.now().toString() + Math.random().toString(), uri: dl.uri, type: 'video', isFav: false, timestamp: Date.now()
              });
      ...
      if (!result.canceled && result.assets.length > 0) {
        ...
        for (const asset of result.assets) {
          ...
          newItems.push({ 
            ...
            uri: securePath, type: isVideo ? 'video' : 'image', isFav: false, title: `Secured Asset` 
          });
        }
          ...
          <View style={styles.vidGrid}>
            {media.map(m => (
              <View key={m.id} style={styles.vidWrapper}>
                ...
                <TouchableOpacity style={styles.vidDelBtn} onPress={() => setConfirmDel(m.id)}><Ionicons name="trash" size={14} color="#FFF" /></TouchableOpacity>
              </View>
            ))}
